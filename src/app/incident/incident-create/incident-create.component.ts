...
export class IncidentCreateComponent implements OnInit {
  ...
  ngOnDestroy() {
    clearTimeout(this.searchTimeout); // Clear the timeout on component destroy
    this.releaseIncidentLock();
  ...
  // // Function to handle API call and form patching
  // async handleSearchText(searchText: string) {
  //   if (searchText.length >= 3) {
  //     let response: any = await this.requestapi.getData(
  //       this.utils.API.SEARCH_PRODUCT_DESCRIPTION_URL +
  //       "?searchText=" +
  //       searchText + '&country=' + this.loginEmployeeCountry + '&userName=' + this.username
  //     );
  //     if (response) {
  ...
  //         productId: "",
  //         productDescription: '',
  //         recommendedAge: (searchText != '') ? 'NA' : '',
  //         articleId: "",
  //         childRecommendedAge: (searchText != '') ? 'NA' : '',
  //       });
  //     }
  //   } else if (searchText.length === 0) {
  //     this.productIdData = [];
  ...
  //   // }
  // }
  private searchTimeout: any; // variable to hold setTimeout reference
private latestSearchText: string = ''; // variable to track the latest keystroke

productIDType(event: any) {
  const searchText = event.target.value;

  // Clear the previous timeout
  clearTimeout(this.searchTimeout);

  // Set the latest search text
  this.latestSearchText = searchText;

  // Delay the API call by 300 milliseconds
  if (searchText.length >= 3) {
    this.searchTimeout = setTimeout(() => {
      if (searchText === this.latestSearchText) {
        this.handleSearchText(searchText);
      }
    }, 300);
  } else if (searchText.length === 0) {
...
async handleSearchText(searchText: string) {
  ...
  onDateChange(event: any) {
    this.incidentMaxDate = this.datepipe.transform(event, "yyyy-MM-dd");
    this.incidentMaxDate = new Date(this.incidentMaxDate);
    this.incidentMaxDate.setDate(this.incidentMaxDate.getDate());
  }
